...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'add_farm') {
        ...
        if ($name && $region && $date_type && $area > 0) {
            ...
            if (!$dup->fetch()) {
                $pdo->prepare("INSERT INTO qw_farm (farmer_id, name, region, total_area_sqm, date_type, description) VALUES (?,?,?,?,?,?)")
                    ->execute([$farmer_id, $name, $region, $area, $date_type, $desc]);
    ...
    if ($act === 'edit_farm') {
        ...
        if ($chk->fetch()) {
            $pdo->prepare("UPDATE qw_farm SET name=?, date_type=?, description=? WHERE farm_id=?")
                ->execute([$name, $date_type, $desc, $farm_id]);
      ...
      <?php foreach ($myFarms as $farm): ?>
        <div class="farm-card">
          ...
          <div class="farm-actions">
            <button class="btn-explore" onclick="openEditFarm(<?= $farm['farm_id'] ?>, '<?= htmlspecialchars(addslashes($farm['name'])) ?>', '<?= htmlspecialchars(addslashes($farm['date_type'])) ?>', '<?= htmlspecialchars(addslashes($farm['description'] ?? '')) ?>')">تعديل</button>
          </div>
        </div>
      <?php endforeach; ?>
    <?php endif; ?>
    ...
    <div class="form-card">
      <form method="POST" action="Farmer.php" id="editFarmForm">
        ...
        <div class="form-group">
          ...
          <select name="date_type" id="edit_date_type" class="form-select">
            <option value="" disabled>اختر نوع التمر</option>
            <option>سكري</option><option>عجوة</option><option>خلاص</option>
            <option>برحي</option><option>مجدول</option><option>سفري</option>
            <option>صفري</option><option>رزيز</option><option>مجهول</option>
          </select>
        </div>
        <div class="form-group">
...
function openEditFarm(id, name, dateType, desc) {
...
document.getElementById('addFarmForm')?.addEventListener('submit', function(e) {
  ...
  if (!name || !region || !area || area <= 0 || !date) {
    e.preventDefault();
