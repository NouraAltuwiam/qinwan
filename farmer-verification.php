...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($farmer_id && in_array($decision, ['verified','rejected'])) {
        ...
        if ($decision === 'rejected' && empty($note)) {
            header('Location: farmer-verification.php?error=note_required');
            exit;
        }
        ...
        try {
            $pdo->prepare("INSERT INTO qw_activity_log (user_id,action_type,entity_type,entity_id,created_at) VALUES (?,?,'farmer',?,NOW())")
                ->execute([$_SESSION['user_id'], $decision . '_farmer', $farmer_id]);
        } catch(Exception $e) {}

        // ✅ إشعار للمزارع
        $farmerUser = $pdo->prepare("
            SELECT u.user_id FROM qw_farmer fr
