...
$wlStmt = $pdo->prepare("
    SELECT w.farm_id, f.name, f.region, f.date_type, MIN(fo.price) AS min_price
    FROM qw_wishlist w
...
// Distinct date types and regions for filters
$dateTypes = array_unique(array_column($farms, 'date_type'));
...
<div class="page" id="page-browse">
  ...
  <div class="page-content">
    ...
    <!-- US-08: Date type multi-filter tags -->
    <div class="palm-filter-tags" id="palmTags">
      ...
      <?php foreach ($dateTypes as $dt): ?>
        <span class="palm-tag" data-palm="<?= htmlspecialchars($dt) ?>" onclick="selectPalm('<?= htmlspecialchars($dt) ?>',this)"><?= htmlspecialchars($dt) ?></span>
      <?php endforeach; ?>
    </div>
    ...
    <div id="view-list">
      <div id="farms-list">
        ...
        ?>
          <div class="farm-card"
               data-name="<?= strtolower($farm['name'].' '.$farm['region'].' '.$farm['date_type']) ?>"
               data-region="<?= htmlspecialchars($farm['region']) ?>"
               data-palm="<?= htmlspecialchars($farm['date_type']) ?>"
            ...
            <div class="farm-card-inner">
              ...
              <div class="farm-details" style="flex:1;">
                ...
                <div class="farm-meta"><?= htmlspecialchars($farm['region']) ?> · <?= htmlspecialchars($farm['date_type']) ?></div>
              ...
              <div class="farm-actions">
                ...
                <button class="btn-explore" onclick="openFarmOffers(<?= $farm['farm_id'] ?>, '<?= htmlspecialchars(addslashes($farm['name'])) ?>', '<?= htmlspecialchars($farm['region']) ?>', '<?= htmlspecialchars($farm['date_type']) ?>')">
                  استكشف العروض
                </button>
              </div>
            </div>
        ...
        <div class="farm-card" style="margin-bottom:12px;">
          <div class="farm-card-inner">
            ...
            <div style="flex:1;">
              ...
              <div class="farm-meta"><?= htmlspecialchars($wf['region']) ?> · <?= htmlspecialchars($wf['date_type']) ?></div>
            ...
            <div class="farm-actions">
              ...
              <button class="btn-explore" onclick="openFarmOffers(<?= $wf['farm_id'] ?>, '<?= htmlspecialchars(addslashes($wf['name'])) ?>', '<?= htmlspecialchars($wf['region']) ?>', '<?= htmlspecialchars($wf['date_type']) ?>')">استكشف</button>
            </div>
          </div>
        </div>
      <?php endforeach; ?>
...
<!-- ============================================================
     Payment Popup (وهمي)
     ============================================================ -->
<div id="paymentPopup" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:800;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:20px;width:100%;max-width:480px;margin:20px;overflow:hidden;box-shadow:0 25px 70px rgba(0,0,0,0.3);">
    
    <!-- Header -->
    <div style="background:linear-gradient(135deg, #2d5f33, #1a3a1e);padding:24px 28px;text-align:center;">
      <div style="font-size:40px;margin-bottom:8px;">💳</div>
      <div style="font-family:'Amiri',serif;font-size:22px;font-weight:700;color:#F2D998;">إتمام عملية الدفع</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.75);margin-top:4px;">بيئة اختبار — لا يتم خصم أي مبلغ حقيقي</div>
    </div>
    ...
    <div style="padding:24px 28px;">
      ...
      <!-- ملخص الطلب -->
      <div style="background:#f7f4ef;border-radius:12px;padding:16px;margin-bottom:20px;border:1px solid #e2d9cf;">
        <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:14px;">
          <span style="color:#8A6F5A;">المزرعة:</span>
          <span style="font-weight:700;color:#5c4634;" id="payFarmName">—</span>
        </div>
        <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:14px;">
          <span style="color:#8A6F5A;">المساحة:</span>
          <span style="font-weight:700;color:#5c4634;" id="payArea">—</span>
        </div>
        <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:14px;">
          <span style="color:#8A6F5A;">المدة:</span>
          <span style="font-weight:700;color:#5c4634;" id="payDuration">—</span>
        </div>
        <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:14px;">
          <span style="color:#8A6F5A;">طريقة الاستلام:</span>
          <span style="font-weight:700;color:#5c4634;" id="payHarvest">—</span>
        </div>
        <hr style="border-color:#e2d9cf;margin:12px 0;">
        <div style="display:flex;justify-content:space-between;font-size:18px;">
          <span style="color:#2d5f33;font-weight:700;">الإجمالي:</span>
          <span style="color:#2d5f33;font-weight:700;font-family:'Amiri',serif;" id="payTotal">—</span>
        </div>
      </div>

      <!-- طرق الدفع -->
      <div style="margin-bottom:20px;">
        <div style="font-size:14px;font-weight:600;color:#5c4634;margin-bottom:12px;">اختر طريقة الدفع:</div>
        
        <label style="display:flex;align-items:center;gap:12px;padding:14px 16px;border:2px solid #e2d9cf;border-radius:12px;cursor:pointer;margin-bottom:10px;transition:all 0.2s;" 
               onmouseenter="this.style.borderColor='#2d5f33';this.style.background='#edf3ee';" 
               onmouseleave="this.style.borderColor='#e2d9cf';this.style.background='transparent';">
          <input type="radio" name="paymentMethod" value="card" checked style="accent-color:#2d5f33;width:18px;height:18px;">
          <span style="font-size:24px;">💳</span>
          <div>
            <div style="font-weight:700;font-size:14px;color:#5c4634;">بطاقة مدى / فيزا</div>
            <div style="font-size:11px;color:#8A6F5A;">تدفع بأمان عبر بوابة الدفع</div>
          </div>
        </label>
        ...
        <label style="display:flex;align-items:center;gap:12px;padding:14px 16px;border:2px solid #e2d9cf;border-radius:12px;cursor:pointer;margin-bottom:10px;transition:all 0.2s;"
               onmouseenter="this.style.borderColor='#2d5f33';this.style.background='#edf3ee';" 
               onmouseleave="this.style.borderColor='#e2d9cf';this.style.background='transparent';">
          <input type="radio" name="paymentMethod" value="apple" style="accent-color:#2d5f33;width:18px;height:18px;">
          <span style="font-size:24px;">🍎</span>
          <div>
            <div style="font-weight:700;font-size:14px;color:#5c4634;">Apple Pay</div>
            <div style="font-size:11px;color:#8A6F5A;">ادفع بنقرة واحدة</div>
          </div>
        </label>

        <label style="display:flex;align-items:center;gap:12px;padding:14px 16px;border:2px solid #e2d9cf;border-radius:12px;cursor:pointer;transition:all 0.2s;"
               onmouseenter="this.style.borderColor='#2d5f33';this.style.background='#edf3ee';" 
               onmouseleave="this.style.borderColor='#e2d9cf';this.style.background='transparent';">
          <input type="radio" name="paymentMethod" value="bank" style="accent-color:#2d5f33;width:18px;height:18px;">
          <span style="font-size:24px;">🏦</span>
          <div>
            <div style="font-weight:700;font-size:14px;color:#5c4634;">تحويل بنكي</div>
            <div style="font-size:11px;color:#8A6F5A;">حوالة محلية عبر بنك الراجحي</div>
          </div>
        </label>
      </div>
      ...
      <div style="display:flex;gap:12px;">
        <button onclick="closePaymentPopup()" 
                style="flex:1;background:transparent;color:#8A6F5A;border:2px solid #d9c8b0;border-radius:30px;padding:13px;font-family:'Noto Naskh Arabic',serif;font-size:14px;font-weight:700;cursor:pointer;transition:all 0.2s;"
                onmouseenter="this.style.borderColor='#8B2A2A';this.style.color='#8B2A2A';"
                onmouseleave="this.style.borderColor='#d9c8b0';this.style.color='#8A6F5A';">
          ✕ إلغاء
        </button>
        <button onclick="confirmPayment()" 
                id="payConfirmBtn"
                style="flex:2;background:linear-gradient(135deg,#2d5f33,#3d7042);color:#fff;border:none;border-radius:30px;padding:13px;font-family:'Noto Naskh Arabic',serif;font-size:15px;font-weight:700;cursor:pointer;transition:all 0.3s;box-shadow:0 4px 15px rgba(45,95,51,0.3);"
                onmouseenter="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(45,95,51,0.4)';"
                onmouseleave="this.style.transform='translateY(0)';this.style.boxShadow='0 4px 15px rgba(45,95,51,0.3)';">
          💳 ادفع الآن وأرسل الطلب
        </button>
      </div>

      <!-- أمان -->
      <div style="display:flex;align-items:center;justify-content:center;gap:14px;margin-top:16px;font-size:11px;color:#b0a090;">
...
<!-- Loading overlay للدفع -->
<div id="paymentLoading" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:900;align-items:center;justify-content:center;flex-direction:column;">
  <div style="background:#fff;border-radius:20px;padding:40px 50px;text-align:center;box-shadow:0 20px 50px rgba(0,0,0,0.3);">
    <div style="font-size:50px;animation:pulse 1s infinite;">🔄</div>
    <div style="font-family:'Amiri',serif;font-size:20px;font-weight:700;color:#2d5f33;margin-top:16px;">جاري معالجة الدفع...</div>
    <div style="font-size:13px;color:#8A6F5A;margin-top:6px;">يرجى الانتظار لحظة</div>
    <div style="margin-top:20px;width:200px;height:4px;background:#e2d9cf;border-radius:2px;overflow:hidden;">
      <div style="height:100%;background:linear-gradient(90deg,#2d5f33,#C8922A,#2d5f33);border-radius:2px;animation:loadBar 1.5s ease-in-out infinite;"></div>
    </div>
  </div>
</div>
...
function openFarmOffers(farmId, name, region, dateType) {
  document.getElementById('offersPageTitle').textContent = name;
  document.getElementById('offersPageMeta').textContent  = region + ' · ' + dateType;
...
function submitInvestment() {
  ...
  // إخفاء نافذة الحصاد
  document.getElementById('harvestPopup').style.display = 'none';

  // ملء بيانات نافذة الدفع
  const durationLabels = { '1_season': 'موسم واحد', '1_year': 'سنة واحدة', '2_years': 'سنتان' };
  const harvestLabels  = { 'receive': 'استلام تمور', 'sell': 'بيع المحصول', 'donate': 'تبرع' };
  
  document.getElementById('payFarmName').textContent = document.getElementById('harvestFarmLabel').textContent.split(' · ')[0] || '—';
  document.getElementById('payArea').textContent     = currentAreaSqm.toLocaleString() + ' م²';
  document.getElementById('payDuration').textContent  = durationLabels[duration] || duration;
  document.getElementById('payHarvest').textContent   = harvestLabels[selectedHarvest] || selectedHarvest;
  document.getElementById('payTotal').textContent     = (currentAreaSqm * currentPrice).toLocaleString() + ' ر.س';
...
function confirmPayment() {
  ...
  // محاكاة وقت معالجة الدفع (2 ثانية)
  setTimeout(() => {
    fetch('investor.php', { method:'POST', body:fd })
      .then(r => r.json())
      .then(d => {
        ...
        if (d.status === 'ok') {
          closeHarvest();
          closeOffersPage();
          showToast('✅ تم الدفع وإرسال طلبك بنجاح! سيراجعه المزارع خلال 7 أيام.');
          setTimeout(() => location.reload(), 2000);
        } else {
          showToast('❌ ' + (d.msg || 'حدث خطأ في معالجة الدفع، حاول مجدداً'));
        }
      })
      .catch(() => {
        document.getElementById('paymentLoading').style.display = 'none';
        showToast('❌ فشل الاتصال، تحقق من الإنترنت وحاول مجدداً');
      });
  }, 2000);
}
  ...
  PHP_FARMS.forEach(farm => {
    ...
    L.marker(jitter, { icon }).addTo(leafletMap).bindPopup(`
      <div dir="rtl" style="text-align:right;font-family:'Noto Naskh Arabic',serif;min-width:200px;">
        ...
        <span style="font-size:12px;color:#666;">${farm.region} · ${farm.date_type}</span><br>
        <span style="font-size:12px;">المساحة: ${parseFloat(farm.total_area_sqm).toLocaleString()} م²</span><br>
        <button onclick="openFarmOffers(${farm.farm_id},'${farm.name}','${farm.region}','${farm.date_type}')"
